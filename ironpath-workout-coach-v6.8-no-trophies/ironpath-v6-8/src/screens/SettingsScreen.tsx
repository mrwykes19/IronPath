    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <Card>
        ...
        <View style={styles.trainingDayRow}>
          {[2, 3, 4, 5].map((days) => (
            <Pressable key={days} onPress={() => updateProfile({ trainingDays: days, trainingSplit: allowedSplitsForDays(days).includes(state.profile.trainingSplit ?? 'auto') ? (state.profile.trainingSplit ?? 'auto') : 'auto', customSplit: (state.profile.customSplit ?? defaultCustomSplit(days)).slice(0, days) })} style={[styles.trainingDayButton, state.profile.trainingDays === days && styles.trainingDayActive]}>

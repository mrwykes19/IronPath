...
import { allowedSplitsForDays, defaultCustomSplit, goalProgramProfiles, splitLabels } from '../data/program';
...
import { BackupRestoreMode, Experience, Goal, SplitDayFocus, TrainingSplit, WorkoutVariety } from '../types';
import { getTrainingPhaseProfile, previewTrainingPhase, trainingPhaseProfiles } from '../engine/weeklyPlanner';
import { BrandMark, PhaseGraphic } from '../components/Visuals';
...
const defaultTrainingWeekdays = (days: number) => days <= 2 ? [1, 4] : days === 3 ? [1, 3, 5] : days === 4 ? [1, 2, 4, 5] : [1, 2, 3, 5, 6];
const weekdaysForCount = (current: number[] | undefined, days: number) => {
  const next = Array.from(new Set((current ?? []).filter((day) => day >= 0 && day <= 6))).sort((a, b) => a - b).slice(0, days);
...
export const SettingsScreen = () => {
  ...
  const selectedTrainingWeekdays = weekdaysForCount(state.profile.trainingWeekdays, state.profile.trainingDays);
  const [workoutDaysDraft, setWorkoutDaysDraft] = useState<number[]>(selectedTrainingWeekdays);
  useEffect(() => setWorkoutDaysDraft(selectedTrainingWeekdays), [state.profile.trainingDays, state.profile.trainingWeekdays?.join(',')]);
  const toggleTrainingWeekday = (day: number) => {
    const next = workoutDaysDraft.includes(day)
      ? workoutDaysDraft.filter((item) => item !== day)
      : [...workoutDaysDraft, day].sort((a, b) => a - b);
    if (next.length > state.profile.trainingDays) return;
    setWorkoutDaysDraft(next);
    if (next.length === state.profile.trainingDays) updateProfile({ trainingWeekdays: next });
  };

  const resolvedDailyCalorieGoal = state.profile.dailyCalorieGoal ?? Math.round((state.profile.weeklyCalorieGoal ?? 6000) / Math.max(1, state.profile.trainingDays));
  const [dailyCaloriesText, setDailyCaloriesText] = useState(String(resolvedDailyCalorieGoal));
  useEffect(() => setDailyCaloriesText(String(resolvedDailyCalorieGoal)), [resolvedDailyCalorieGoal]);
  const saveDailyCalories = () => {
    const parsed = Number(dailyCaloriesText);
    const goal = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed) : resolvedDailyCalorieGoal;
    setDailyCaloriesText(String(goal));
    updateProfile({ dailyCalorieGoal: goal, weeklyCalorieGoal: goal * Math.max(1, state.profile.trainingDays) });
  };
  const resolvedCardioDaysGoal = Math.max(0, Math.min(7, state.profile.weeklyCardioDaysGoal ?? 3));
  const [cardioDaysText, setCardioDaysText] = useState(String(resolvedCardioDaysGoal));
  useEffect(() => setCardioDaysText(String(resolvedCardioDaysGoal)), [resolvedCardioDaysGoal]);
  const saveCardioDays = () => {
    const parsed = Number(cardioDaysText);
    const goal = Number.isFinite(parsed) ? Math.max(0, Math.min(7, Math.round(parsed))) : resolvedCardioDaysGoal;
    setCardioDaysText(String(goal));
    updateProfile({ weeklyCardioDaysGoal: goal });
  };
  const resolvedDailyActiveMinutesGoal = state.profile.dailyActiveMinutesGoal ?? Math.max(1, Math.round((state.profile.weeklyActiveMinutesGoal ?? 240) / Math.max(1, state.profile.trainingDays)));
  const [activeMinutesText, setActiveMinutesText] = useState(String(resolvedDailyActiveMinutesGoal));
  useEffect(() => setActiveMinutesText(String(resolvedDailyActiveMinutesGoal)), [resolvedDailyActiveMinutesGoal]);
  const saveActiveMinutes = () => {
    const parsed = Number(activeMinutesText);
    const goal = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed) : resolvedDailyActiveMinutesGoal;
    setActiveMinutesText(String(goal));
    updateProfile({ dailyActiveMinutesGoal: goal, weeklyActiveMinutesGoal: goal * Math.max(1, state.profile.trainingDays) });
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <Card style={styles.heroCard}>
        <View style={styles.programRow}><BrandMark compact /><View style={styles.flex}><Text style={styles.cardEyebrow}>ACTIVE PROGRAM</Text><Text style={styles.programName}>{goalProgramProfiles[state.profile.goal].name}</Text><Text style={styles.copy}>{goalProgramProfiles[state.profile.goal].description}</Text></View></View>
      ...
      <Card>
        ...
        <View style={styles.trainingDayRow}>
          {[2, 3, 4, 5].map((days) => (
            <Pressable key={days} onPress={() => updateProfile({ trainingDays: days, trainingWeekdays: weekdaysForCount(state.profile.trainingWeekdays, days), trainingSplit: allowedSplitsForDays(days).includes(state.profile.trainingSplit ?? 'auto') ? (state.profile.trainingSplit ?? 'auto') : 'auto', customSplit: (state.profile.customSplit ?? defaultCustomSplit(days)).slice(0, days) })} style={[styles.trainingDayButton, state.profile.trainingDays === days && styles.trainingDayActive]}>
        ...
        <Text style={styles.dayPickerHint}>Choose the exact days shown in your weekly preview. Tap selected days to remove them, then choose replacements.</Text>
        <View style={styles.weekdayPicker}>
          {weekdayNames.map((name, day) => {
            const active = workoutDaysDraft.includes(day);
            return <Pressable key={name} onPress={() => toggleTrainingWeekday(day)} style={[styles.weekdayButton, active && styles.weekdayButtonActive]}><Text style={[styles.weekdayText, active && styles.weekdayTextActive]}>{name}</Text></Pressable>;
          })}
        </View>
        <Text style={styles.selectedDaysText}>{workoutDaysDraft.length === state.profile.trainingDays ? workoutDaysDraft.map((day) => weekdayNames[day]).join(' · ') : `Select ${state.profile.trainingDays - workoutDaysDraft.length} more day${state.profile.trainingDays - workoutDaysDraft.length === 1 ? '' : 's'}`}</Text>
        <View style={styles.fieldRow}>
          <View style={styles.field}><Text style={styles.label}>Weight increments</Text><View style={styles.input}><Text style={styles.selectedDaysText}>5 {state.profile.unit} for all weighted exercises</Text></View></View>
        </View>
      ...
      <Card>
        <Text style={styles.cardTitle}>Training split</Text>
        <Text style={styles.copy}>Choose how IronPath divides muscle groups across your training days. Changing the split rebuilds future workouts but leaves completed history untouched.</Text>
        <View style={styles.splitGrid}>
          {allowedSplitsForDays(state.profile.trainingDays).map((split) => {
            const active = (state.profile.trainingSplit ?? 'auto') === split;
            return (
              <Pressable key={split} onPress={() => updateProfile({ trainingSplit: split, customSplit: split === 'custom' ? (state.profile.customSplit?.slice(0, state.profile.trainingDays) ?? defaultCustomSplit(state.profile.trainingDays)) : state.profile.customSplit })} style={[styles.splitOption, active && styles.splitOptionActive]}>
                <Text style={[styles.splitOptionTitle, active && styles.splitOptionTitleActive]}>{splitLabels[split]}</Text>
                <Text style={[styles.splitOptionCopy, active && styles.splitOptionCopyActive]}>{split === 'auto' ? 'IronPath chooses the recommended split.' : split === 'full-body' ? 'Most major muscle groups each session.' : split === 'upper-lower' ? 'Alternating upper- and lower-body days.' : split === 'push-pull-legs' ? 'Pressing, pulling, and leg-focused days.' : split === 'hybrid' ? 'Upper/lower plus push, pull, and legs.' : split === 'body-part' ? 'More focused muscle-group sessions.' : 'Choose the focus for each training day.'}</Text>
              </Pressable>
            );
          })}
        </View>
        {(state.profile.trainingSplit ?? 'auto') === 'custom' ? (
          <View style={styles.customSplitBox}>
            <Text style={styles.label}>Custom training days</Text>
            {Array.from({ length: state.profile.trainingDays }, (_, index) => {
              const focuses: SplitDayFocus[] = ['full-body', 'upper', 'lower', 'push', 'pull', 'legs', 'chest-arms', 'back-biceps', 'shoulders-core'];
              const selected = state.profile.customSplit?.[index] ?? defaultCustomSplit(state.profile.trainingDays)[index] ?? 'full-body';
              return (
                <View key={index} style={styles.customSplitDay}>
                  <Text style={styles.customSplitDayLabel}>DAY {index + 1}</Text>
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.customSplitChoices}>
                    {focuses.map((focus) => <Pressable key={focus} onPress={() => { const next = [...(state.profile.customSplit ?? defaultCustomSplit(state.profile.trainingDays))]; next[index] = focus; updateProfile({ customSplit: next }); }} style={[styles.customSplitChoice, selected === focus && styles.customSplitChoiceActive]}><Text style={[styles.customSplitChoiceText, selected === focus && styles.customSplitChoiceTextActive]}>{focus.replace(/-/g, ' ')}</Text></Pressable>)}
                  </ScrollView>
                </View>
              );
            })}
        ...
        <Text style={styles.planHint}>Active split: {splitLabels[state.profile.trainingSplit ?? 'auto']}. You can still edit individual workouts in My Training Plan.</Text>
      </Card>

      <Card>
        <Text style={styles.cardTitle}>Weekly goals</Text>
        <Text style={styles.copy}>Set the targets shown on Today. Total workouts follows your selected workouts-per-week plan.</Text>
        <View style={styles.goalTargetSummary}>
          <View style={styles.goalTargetFixed}><Text style={styles.goalTargetValue}>{state.profile.trainingDays}</Text><Text style={styles.goalTargetLabel}>TOTAL WORKOUTS</Text></View>
          <Text style={styles.goalTargetNote}>Change this through “Workouts per week” above.</Text>
        </View>
        <View style={styles.fieldRow}>
          <View style={styles.field}><Text style={styles.label}>Cardio days</Text><TextInput value={cardioDaysText} onChangeText={setCardioDaysText} onBlur={saveCardioDays} keyboardType="number-pad" placeholder="3" placeholderTextColor={colors.mutedSoft} style={styles.input} /></View>
          <View style={styles.field}><Text style={styles.label}>Daily calorie goal</Text><TextInput value={dailyCaloriesText} onChangeText={setDailyCaloriesText} onBlur={saveDailyCalories} keyboardType="number-pad" placeholder="1500" placeholderTextColor={colors.mutedSoft} style={styles.input} /></View>
        </View>
        <View style={styles.fieldRow}>
          <View style={styles.field}><Text style={styles.label}>Daily active minutes</Text><TextInput value={activeMinutesText} onChangeText={setActiveMinutesText} onBlur={saveActiveMinutes} keyboardType="number-pad" placeholder="60" placeholderTextColor={colors.mutedSoft} style={styles.input} /></View>
          <View style={styles.field}><Text style={styles.label}>Goal period</Text><View style={styles.readOnlyField}><Text style={styles.readOnlyValue}>7 days</Text></View></View>
        </View>
        <Text style={styles.planHint}>Weekly targets: {(resolvedDailyCalorieGoal * state.profile.trainingDays).toLocaleString()} calories and {(resolvedDailyActiveMinutesGoal * state.profile.trainingDays).toLocaleString()} active minutes. Cardio days count distinct completed cardio days.</Text>
      </Card>
            ...
            {trainingPhaseProfiles.map((phase) => {
              ...
              return (
                <Pressable key={phase.phase} onPress={() => { setSelectedPhase(phase.phase); setExpandedPhaseWorkout(undefined); }} style={[styles.phaseTab, selected && styles.phaseTabSelected]}>
                  <PhaseGraphic phase={phase.phase} active={selected} />
...
const styles = StyleSheet.create({
  ...
  goalTargetSummary: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: colors.panelSoft, borderWidth: 1, borderColor: colors.border, borderRadius: 14, padding: 11 },
  goalTargetFixed: { minWidth: 82, alignItems: 'center', justifyContent: 'center', borderRightWidth: 1, borderRightColor: colors.border, paddingRight: 12 },
  goalTargetValue: { color: colors.accent, fontSize: 24, fontWeight: '900' },
  goalTargetLabel: { color: colors.muted, fontSize: 7, fontWeight: '900', marginTop: 2 },
  goalTargetNote: { flex: 1, color: colors.muted, fontSize: 10, lineHeight: 15 },
  readOnlyField: { minHeight: 48, borderRadius: 14, backgroundColor: colors.panelSoft, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 13, justifyContent: 'center' },
  readOnlyValue: { color: colors.muted, fontSize: 16, fontWeight: '700' },
  ...
  phaseTabs: { flexDirection: 'row', gap: 8 },
  phaseTab: { flex: 1, minHeight: 114, borderRadius: 16, backgroundColor: colors.panelRaised, borderWidth: 1, borderColor: colors.border, alignItems: 'center', justifyContent: 'center', position: 'relative', paddingVertical: 8, paddingHorizontal: 4 },
  ...
  phaseTabLabel: { color: colors.text, fontSize: 10, fontWeight: '900', marginTop: 6 },
  ...
  splitOption: { width: '48.5%', minHeight: 88, borderRadius: 14, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.panelRaised, padding: 11 },
  splitOptionActive: { backgroundColor: colors.accentDeep, borderColor: colors.accent },
  splitOptionTitle: { color: colors.text, fontSize: 12, fontWeight: '900' },
  splitOptionTitleActive: { color: colors.accent },
  splitOptionCopy: { color: colors.muted, fontSize: 9, lineHeight: 13, marginTop: 5 },
  splitOptionCopyActive: { color: colors.text },
  customSplitBox: { gap: 10, backgroundColor: colors.panelSoft, borderWidth: 1, borderColor: colors.border, borderRadius: 14, padding: 11 },
  customSplitDay: { gap: 6 },
  customSplitDayLabel: { color: colors.muted, fontSize: 8, fontWeight: '900' },
  customSplitChoices: { gap: 6, paddingRight: 8 },
  customSplitChoice: { borderRadius: 999, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.panelRaised, paddingHorizontal: 10, paddingVertical: 7 },
  customSplitChoiceActive: { backgroundColor: colors.accent, borderColor: colors.accentStrong },
  customSplitChoiceText: { color: colors.muted, fontSize: 9, fontWeight: '800', textTransform: 'capitalize' },
  customSplitChoiceTextActive: { color: colors.black },
  dayPickerHint: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: -4 },
  weekdayPicker: { flexDirection: 'row', gap: 5 },
  weekdayButton: { flex: 1, minHeight: 42, borderRadius: 12, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.panelSoft, alignItems: 'center', justifyContent: 'center' },
  weekdayButtonActive: { borderColor: colors.accent, backgroundColor: colors.accentDark },
  weekdayText: { color: colors.muted, fontSize: 9, fontWeight: '900' },
  weekdayTextActive: { color: colors.accent },
  selectedDaysText: { color: colors.accent, fontSize: 10, fontWeight: '800', textAlign: 'center', marginTop: -2 },
});

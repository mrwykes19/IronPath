...
export const TodayScreen = ({ openTrain, openHistory }: { openTrain: () => void; openHistory: () => void }) => {
  const { state, startWorkout, startCustomWorkout, updatePlannedWorkout, updatePlannedExercise, movePlannedExercise, removePlannedExercise, addPlannedExercise } = useApp();
  ...
  const calorieGoal = Math.max(1, state.profile.weeklyCalorieGoal ?? 1500);
  const cardioDaysGoal = Math.max(1, state.profile.weeklyCardioDaysGoal ?? 3);
  const activeMinutesGoal = Math.max(1, state.profile.weeklyActiveMinutesGoal ?? 240);
  const workoutProgress = Math.min(100, Math.round((summary.workouts / workoutGoal) * 100));
  const calorieProgress = Math.min(100, Math.round((summary.calories / calorieGoal) * 100));
  const cardioDaysProgress = Math.min(100, Math.round((summary.cardioDays / cardioDaysGoal) * 100));
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.openSection}>
        ...
        <View style={styles.goalGrid}>
          <View style={styles.goalCell}><ProgressRing value={`${summary.workouts}/${workoutGoal}`} label="Total workouts" progress={workoutProgress} /></View>
          <View style={styles.goalCell}><ProgressRing value={`${summary.cardioDays}/${cardioDaysGoal}`} label="Cardio days" progress={cardioDaysProgress} /></View>
          <View style={styles.goalCell}><ProgressRing value={`${compactNumber(summary.calories)}/${compactNumber(calorieGoal)}`} label="Calories burned" progress={calorieProgress} /></View>
          <View style={styles.goalCell}><ProgressRing value={`${summary.activeMinutes}/${activeMinutesGoal}`} label="Active minutes" progress={activeMinutesProgress} /></View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  goalGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, paddingHorizontal: 4 },
  goalCell: { width: '47.5%', alignItems: 'center', paddingVertical: 5 },

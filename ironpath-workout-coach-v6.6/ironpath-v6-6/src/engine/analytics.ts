...
export const achievements = (state: AppState) => {
  ...
  return [
    ...
    { id: 'workouts', title: 'Workout Club', value: `${completed.length + completedCardio.length}`, detail: completed.length + completedCardio.length >= 100 ? '100 workouts completed' : `${Math.max(0, 100 - (completed.length + completedCardio.length))} to 100 workouts`, unlocked: completed.length + completedCardio.length >= 10 },

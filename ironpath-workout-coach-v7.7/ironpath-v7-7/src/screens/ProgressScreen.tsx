...
import { getExerciseHistory, muscleVolume, personalRecords, weeklySummary } from '../engine/analytics';
...
type ExerciseProgress = { bestE1rm: number; bestWeight: number; lastWeight: number; previousWeight: number; change: number; sessions: number; values: number[] };
...
export const ProgressScreen = () => {
  ...
  const exerciseProgress = useMemo(() => {
    ...
    completed.forEach((session) => session.exercises.forEach((exercise) => {
      ...
      if (!best) return;
      const current = map.get(exercise.exerciseId) ?? { bestE1rm: 0, bestWeight: 0, lastWeight: 0, previousWeight: 0, change: 0, sessions: 0, values: [] };
      const sessionBestWeight = sets.reduce((value, set) => Math.max(value, set.weight), 0);
      const previousWeight = current.lastWeight;
      map.set(exercise.exerciseId, {
        ...
        previousWeight,
        change: previousWeight ? sessionBestWeight - previousWeight : 0,
        sessions: current.sessions + 1,
        values: [...current.values, sessionBestWeight]
      });
    }));
  ...
  if (selectedExerciseId) {
    ...
    const latestActualWeight = history.at(-1)?.bestSet.weight ?? 0;
    const previousActualWeight = history.at(-2)?.bestSet.weight ?? 0;
    const weightChange = previousActualWeight ? latestActualWeight - previousActualWeight : 0;
    ...
    return (
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Card style={styles.heroCard}>
          ...
          <Text style={styles.bigMetric}>{latestActualWeight || '—'} {state.profile.unit}</Text>
          <Text style={styles.positiveDetail}>{history.length > 1 ? `${weightChange > 0 ? '+' : ''}${weightChange} ${state.profile.unit} since last workout` : history.length ? 'First logged workout' : 'Complete this lift to begin tracking'}</Text>
          {history.length ? <TrendBars values={history.slice(-7).map((item) => item.bestSet.weight)} labels={history.slice(-7).map((item) => new Date(item.date).toLocaleDateString(undefined, { month: 'numeric', day: 'numeric' }))} /> : <Text style={styles.empty}>No strength data yet.</Text>}
        </Card>
      ...
      {tab === 'overview' ? (
        <>
          ...
          <View style={styles.trendGrid}>
            {(primaryTrends.length ? primaryTrends : [['bench-press', { bestE1rm: 0, bestWeight: 0, lastWeight: 0, previousWeight: 0, change: 0, sessions: 0, values: [0, 0] }] as [string, ExerciseProgress], ['back-squat', { bestE1rm: 0, bestWeight: 0, lastWeight: 0, previousWeight: 0, change: 0, sessions: 0, values: [0, 0] }] as [string, ExerciseProgress], ['trap-bar-deadlift', { bestE1rm: 0, bestWeight: 0, lastWeight: 0, previousWeight: 0, change: 0, sessions: 0, values: [0, 0] }] as [string, ExerciseProgress]]).slice(0, 3).map(([exerciseId, data]) => (
              <Pressable key={exerciseId} style={styles.trendCard} onPress={() => setSelectedExerciseId(exerciseId)}>
                ...
                <Text style={styles.trendValue}>{data.lastWeight || '—'} <Text style={styles.trendUnit}>{state.profile.unit}</Text></Text>
                <Text style={styles.trendGain}>{data.sessions > 1 ? `${data.change > 0 ? '+' : ''}${data.change} ${state.profile.unit} since last workout` : data.sessions ? 'First logged workout' : 'Start tracking'}</Text>

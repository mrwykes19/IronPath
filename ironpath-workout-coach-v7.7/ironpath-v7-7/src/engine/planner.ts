...
import { AppState, LoggedSet, WorkoutSession, WorkoutTemplate } from '../types';
...
export const getNextTemplate = (state: AppState) => {
  ...
  const program = plan?.workouts.length ? plan.workouts : getProgramForGoal(prepared.profile.goal, prepared.profile.trainingDays, prepared.profile.trainingSplit ?? 'auto', prepared.profile.customSplit);
  return program[state.nextTemplateIndex % program.length] ?? program[0];
};

export const createWorkoutSessionFromTemplate = (prepared: AppState, template: WorkoutTemplate, isCustom = false): WorkoutSession => {
  const blockWeek = isCustom ? 3 : (prepared.weeklyPlan?.blockWeek ?? 1);
  ...
  return {
    ...
    exercises: template.exercises.map((item) => {
      ...
      const recommendation = {
        ...
        reason: isCustom
          ? `Custom session: use your current working recommendation and adjust based on today's performance. ${baseRecommendation.reason}`
          : blockWeek === 1
...
export const createWorkoutSession = (state: AppState): WorkoutSession => {
  ...
  return createWorkoutSessionFromTemplate(prepared, template, false);
};

export const createCustomWorkoutSession = (state: AppState, template: WorkoutTemplate): WorkoutSession =>
  createWorkoutSessionFromTemplate(state, template, true);

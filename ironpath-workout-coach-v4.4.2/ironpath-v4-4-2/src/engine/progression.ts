...
export const getRecommendation = (state: AppState, exerciseId: string): WeightRecommendation => {
  ...
  if (!latest) {
    ...
    const startingWeight = roundToIncrement(definition.defaultWeight * factor, definition.defaultIncrement || 1);
  ...
  const increment = definition.compound
    ? definition.primaryMuscle === 'Quads' || definition.primaryMuscle === 'Posterior chain' || definition.primaryMuscle === 'Hamstrings'
      ? state.profile.lowerIncrement
      : state.profile.upperIncrement
    : definition.defaultIncrement;
  ...
  if (twoMisses) {
    const reduced = roundToIncrement(latestWeight * 0.9, increment || definition.defaultIncrement || 1);
  ...
  if (successes >= 2 || (isTopRangeSuccess(latest) && averageRir >= 2.5)) {
    const increased = roundToIncrement(latestWeight + (increment || definition.defaultIncrement || 5), increment || 1);
    return {
      kind: 'increase',
      weight: increased,

...
const initialState: AppState = {
  profile: {
    ...
    trainingWeekdaysSundayFirst: true,
    sessionMinutes: 60,
    currentWeight: 185,
    goalWeight: 175,
    upperIncrement: 5,
...
// v7.0 stored workout-day indexes with Monday as 0. Convert those values once
// so v7.1 and later consistently use JavaScript's Sunday-first day indexes.
const migrateTrainingWeekdaysToSundayFirst = (profile: UserProfile | undefined) => {
  const weekdays = profile?.trainingWeekdays;
  if (!weekdays?.length || profile?.trainingWeekdaysSundayFirst) return weekdays;
  return weekdays.map((day) => (day + 1) % 7);
};
...
const mergeStoredState = (rawStored: AppState): AppState => {
  ...
  profile: { ...initialState.profile, ...stored.profile, trainingWeekdays: normalizeTrainingWeekdays(migrateTrainingWeekdaysToSundayFirst(stored.profile), stored.profile?.trainingDays ?? initialState.profile.trainingDays), trainingWeekdaysSundayFirst: true, upperIncrement: 5, lowerIncrement: 5 },
...
export const AppProvider = ({ children }: PropsWithChildren) => {
  ...
  const value = useMemo<AppContextValue>(() => ({
    ...
    finishCardio: (distanceMiles, notes) => setState((current) => {
      ...
      const weightKg = current.profile.unit === 'kg' ? current.profile.currentWeight : current.profile.currentWeight * 0.453592;
      const metByDevice = { 'treadmill-run': 9.8, 'treadmill-walk': 4.3, bike: 7.0, elliptical: 5.0, rower: 7.0, 'stair-climber': 8.8 } as const;
      const estimatedCalories = Math.max(1, Math.round(((metByDevice[current.activeCardio.device] * 3.5 * weightKg) / 200) * minutes));
      const completed = { ...current.activeCardio, completedAt, durationSeconds, distanceMiles: distanceMiles && distanceMiles > 0 ? distanceMiles : undefined, calories: estimatedCalories, notes: notes?.trim() || undefined, entrySource: 'timer' as const };
    ...
    updateProfile: (patch) => setState((current) => {
      ...
      const next = { ...current, profile: { ...current.profile, ...patch, trainingWeekdays: normalizeTrainingWeekdays(requestedWeekdays, nextTrainingDays), trainingWeekdaysSundayFirst: true, upperIncrement: 5, lowerIncrement: 5 } };

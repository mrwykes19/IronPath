...
import { AchievementGraphic, ActivityGraphic, EmptyStateGraphic } from '../components/Visuals';


const achievementImages: Record<string, any> = {
  'first-workout': require('../../assets/v68/achievements/first_workout.png'),
  'cardio-starter': require('../../assets/v68/achievements/cardio_starter.png'),
  'three-day-streak': require('../../assets/v68/achievements/three_day_streak.png'),
  'first-week': require('../../assets/v68/achievements/first_week.png'),
  'thousand-calories': require('../../assets/v68/achievements/thousand_calories.png'),
  'seven-day-streak': require('../../assets/v68/achievements/seven_day_streak.png'),
  'ten-workouts': require('../../assets/v68/achievements/ten_workouts.png'),
  'new-max': require('../../assets/v68/achievements/new_max.png'),
  'goal-crusher': require('../../assets/v68/achievements/goal_crusher.png'),
  'thirty-day-streak': require('../../assets/v68/achievements/thirty_day_streak.png')
};
...
export const ProgressScreen = () => {
  ...
  const cardio = (state.cardioSessions ?? []).filter((session) => session.completedAt);
  const cardioWithDistance = cardio.filter((session) => typeof session.distanceMiles === 'number' && session.distanceMiles > 0);
  const recentCardio = cardioWithDistance.slice(-6);
  const totalCardioDistance = cardioWithDistance.reduce((sum, session) => sum + (session.distanceMiles ?? 0), 0);
  const bestCardioDistance = cardioWithDistance.reduce((best, session) => Math.max(best, session.distanceMiles ?? 0), 0);
  const latestCardioDistance = cardioWithDistance.at(-1)?.distanceMiles ?? 0;
  const previousCardioDistance = cardioWithDistance.at(-2)?.distanceMiles ?? 0;
  const cardioGain = latestCardioDistance && previousCardioDistance ? latestCardioDistance - previousCardioDistance : 0;
  const totalCardioCalories = cardio.reduce((sum, session) => sum + (session.calories ?? 0), 0);
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Card>
          ...
          <View style={styles.recordGrid}>
            <View style={styles.recordCell}><AchievementGraphic kind="pr" /><Text style={styles.recordValue}>{heaviest || '—'} {state.profile.unit}</Text><Text style={styles.recordLabel}>Heaviest set</Text></View>
            <View style={styles.recordCell}><AchievementGraphic kind="workouts" /><Text style={styles.recordValue}>{best ? Math.round(best) : '—'} {state.profile.unit}</Text><Text style={styles.recordLabel}>Best estimated 1RM</Text></View>
          </View>
        </Card>
  ...
  return (
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <View style={styles.achievementHeader}><View><Text style={styles.sectionEyebrow}>ACHIEVEMENTS</Text><Text style={styles.achievementSubtitle}>{badges.filter((badge) => badge.unlocked).length} earned · swipe to explore</Text></View><Text style={styles.achievementCount}>{badges.filter((badge) => badge.unlocked).length}/{badges.length}</Text></View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.achievementRail}>
        {badges.map((badge) => (
          <View key={badge.id} style={[styles.achievementCard, !badge.unlocked && styles.achievementLocked]}>
            <Image source={achievementImages[badge.id]} style={styles.achievementImage} resizeMode="contain" />
            {!badge.unlocked ? <View style={styles.lockPill}><Text style={styles.lockPillText}>LOCKED</Text></View> : <View style={styles.earnedPill}><Text style={styles.earnedPillText}>EARNED</Text></View>}
            <Text style={styles.achievementTitle} numberOfLines={1}>{badge.title}</Text>
            <Text style={styles.achievementProgress}>{badge.unlocked ? 'Achievement unlocked' : `${Math.min(badge.current, badge.target).toLocaleString()} / ${badge.target.toLocaleString()}`}</Text>
          </View>
        ))}
      </ScrollView>
      ...
      {tab === 'overview' ? (
        <>
          ...
          <Card style={styles.heroCard}>
            <View style={styles.summaryGrid}>
              <View style={styles.summaryRow}>
                <MetricTile label="WORKOUTS" value={`${summary.workouts}`} detail={`${summary.strengthWorkouts} strength · ${summary.cardioWorkouts} cardio`} />
                <MetricTile label="VOLUME" value={`${summary.volumeChangePercent >= 0 ? '+' : ''}${summary.volumeChangePercent}%`} detail="vs last week" />
              </View>
              <View style={styles.summaryRow}>
                <MetricTile label="CALORIES" value={summary.calories ? `${summary.calories}` : '—'} detail="strength + cardio" />
          ...
          <Text style={styles.sectionEyebrow}>CARDIO PROGRESS</Text>
          <Card>
            <View style={styles.cardioHeader}>
              <ActivityGraphic device="treadmill-run" active size={44} />
              <View style={styles.flex}><Text style={styles.cardTitle}>Distance per session</Text><Text style={styles.detailCopy}>{cardioWithDistance.length ? `${cardioWithDistance.length} tracked cardio sessions` : 'Cardio sessions from Today will appear here.'}</Text></View>
              <View style={styles.cardioBadge}><Text style={styles.cardioBadgeValue}>{latestCardioDistance ? latestCardioDistance.toFixed(2) : '—'}</Text><Text style={styles.cardioBadgeLabel}>LATEST MI</Text></View>
            </View>
            {recentCardio.length ? <TrendBars values={recentCardio.map((session) => session.distanceMiles ?? 0)} labels={recentCardio.map((session) => new Date(session.completedAt!).toLocaleDateString(undefined, { month: 'numeric', day: 'numeric' }))} /> : null}
            <View style={styles.cardioStats}>
              <View style={styles.cardioStat}><Text style={styles.cardioStatLabel}>TOTAL DISTANCE</Text><Text style={styles.cardioStatValue}>{totalCardioDistance.toFixed(1)} mi</Text></View>
              <View style={styles.cardioStat}><Text style={styles.cardioStatLabel}>BEST SESSION</Text><Text style={styles.cardioStatValue}>{bestCardioDistance ? `${bestCardioDistance.toFixed(2)} mi` : '—'}</Text></View>
              <View style={styles.cardioStat}><Text style={styles.cardioStatLabel}>CARDIO CALORIES</Text><Text style={styles.cardioStatValue}>{totalCardioCalories ? totalCardioCalories.toLocaleString() : '—'}</Text></View>
              <View style={styles.cardioStat}><Text style={styles.cardioStatLabel}>LAST CHANGE</Text><Text style={[styles.cardioStatValue, cardioGain > 0 && styles.cardioPositive]}>{cardioGain ? `${cardioGain > 0 ? '+' : ''}${cardioGain.toFixed(2)} mi` : '—'}</Text></View>
            </View>
          </Card>
...
const styles = StyleSheet.create({
  ...
  cardioHeader: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  cardioBadge: { minWidth: 76, borderRadius: 13, backgroundColor: colors.accentDeep, borderWidth: 1, borderColor: colors.borderStrong, paddingHorizontal: 10, paddingVertical: 8, alignItems: 'center' },
  cardioBadgeValue: { color: colors.accent, fontSize: 18, fontWeight: '900' },
  cardioBadgeLabel: { color: colors.muted, fontSize: 7, fontWeight: '900', marginTop: 2 },
  cardioStats: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 },
  cardioStat: { flexBasis: '47%', flexGrow: 1, minHeight: 62, borderRadius: 12, backgroundColor: colors.panelSoft, borderWidth: 1, borderColor: colors.border, padding: 9 },
  cardioStatLabel: { color: colors.mutedSoft, fontSize: 7, fontWeight: '900', letterSpacing: 0.5 },
  cardioStatValue: { color: colors.text, fontSize: 13, fontWeight: '900', marginTop: 7 },
  cardioPositive: { color: colors.accent },
  ...
  empty: { color: colors.muted, paddingVertical: 10, textAlign: 'center', fontSize: 11 },
  ...
  achievementHeader: { flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', marginTop: 2, marginBottom: 10 },
  achievementSubtitle: { color: colors.muted, fontSize: 10, marginTop: 3 },
  achievementCount: { color: colors.accent, fontSize: 12, fontWeight: '900', borderWidth: 1, borderColor: colors.borderStrong, backgroundColor: colors.accentDeep, borderRadius: 999, paddingHorizontal: 9, paddingVertical: 4 },
  achievementRail: { gap: 11, paddingRight: 18, paddingBottom: 18 },
  achievementCard: { width: 154, borderRadius: 18, borderWidth: 1, borderColor: colors.borderStrong, backgroundColor: colors.panel, padding: 9, position: 'relative' },
  achievementLocked: { opacity: 0.42, borderColor: colors.border },
  achievementImage: { width: 134, height: 134, alignSelf: 'center' },
  achievementTitle: { color: colors.text, fontSize: 12, fontWeight: '900', marginTop: 5 },
  achievementProgress: { color: colors.muted, fontSize: 9, marginTop: 3 },
  earnedPill: { position: 'absolute', top: 8, right: 8, borderRadius: 999, backgroundColor: '#5C4510', borderWidth: 1, borderColor: '#F6C84A', paddingHorizontal: 6, paddingVertical: 3 },
  earnedPillText: { color: '#FFE69A', fontSize: 6, fontWeight: '900', letterSpacing: 0.7 },
  lockPill: { position: 'absolute', top: 8, right: 8, borderRadius: 999, backgroundColor: colors.panelRaised, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 6, paddingVertical: 3 },
  lockPillText: { color: colors.muted, fontSize: 6, fontWeight: '900', letterSpacing: 0.7 },
  badgeImage: { width: 118, height: 118, alignSelf: 'center' },

...
const trainingDayIndexes = (days: number) => {
...
export const TodayScreen = ({ openTrain, openHistory, openSettings }: { openTrain: () => void; openHistory: () => void; openSettings: () => void }) => {
  ...
  const workoutDayIndexes = trainingDayIndexes(state.profile.trainingDays);
  const schedule = Array.from({ length: 7 }, (_, dayIndex) => {
    const workoutIndex = workoutDayIndexes.indexOf(dayIndex);
    const date = new Date(weekStart);
    date.setDate(weekStart.getDate() + dayIndex);
    return { dayIndex, date, workoutIndex, workout: workoutIndex >= 0 ? plannedWorkouts[workoutIndex] : undefined };
  });
      ...
      <View style={styles.openSection}>
        <View style={styles.sectionHeader}>
          <View>
            ...
            <Text style={styles.sectionSubtitle}>Tap any day to preview the plan</Text>
          </View>
        ...
        <Text style={styles.scheduleHint}>{plannedWorkouts.length} planned strength sessions · rest or cardio on open days</Text>
      </View>

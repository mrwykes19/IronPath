  ...
  return (
    <Card style={styles.card}>
      <Pressable style={styles.header} onPress={() => setExpanded((value) => !value)}>
        <View style={styles.icon}><ActivityGraphic device={state.activeCardio?.device ?? selected} active size={34} /></View>
              ...
              <View style={styles.deviceGrid}>
                {options.map((item) => (
                  <Pressable key={item.id} onPress={() => setSelected(item.id)} style={[styles.device, selected === item.id && styles.deviceActive]}>
                    <ActivityGraphic device={item.id} active={selected === item.id} />

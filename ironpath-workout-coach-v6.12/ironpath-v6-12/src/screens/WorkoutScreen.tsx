...
export const WorkoutScreen = ({ goToday }: { goToday: () => void }) => {
  ...
  const alternatives = exercise?.substitutions.map((id) => exercisesById[id]).filter((item): item is NonNullable<typeof item> => Boolean(item)) ?? [];
        ...
        <>
          <Card style={styles.exerciseHero}>
            ...
            <View style={styles.controlRow}>
              <View style={styles.controlBlock}><Text style={styles.controlLabel}>WEIGHT ({state.profile.unit.toUpperCase()})</Text><StepControl compact value={selectedSet.weight} onChange={(weight) => updateSet(exerciseIndex, setIndex, { weight })} step={exercise?.defaultIncrement || 5} /></View>

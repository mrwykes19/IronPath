  ...
  return (
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <Card>
        ...
        <View style={styles.fieldRow}>
          <View style={styles.field}><Text style={styles.label}>Cardio days</Text><TextInput value={(state.profile.weeklyCardioDaysGoal ?? 3).toString()} onChangeText={(value) => updateProfile({ weeklyCardioDaysGoal: Math.max(1, Math.min(7, Number(value) || 1)) })} keyboardType="number-pad" style={styles.input} /></View>
        ...
        <View style={styles.fieldRow}>
          <View style={styles.field}><Text style={styles.label}>Active minutes</Text><TextInput value={(state.profile.weeklyActiveMinutesGoal ?? 240).toString()} onChangeText={(value) => updateProfile({ weeklyActiveMinutesGoal: Math.max(30, Number(value) || 30) })} keyboardType="number-pad" style={styles.input} /></View>
        ...
        <Text style={styles.planHint}>Weekly calorie target: {(resolvedDailyCalorieGoal * state.profile.trainingDays).toLocaleString()} ({resolvedDailyCalorieGoal.toLocaleString()} per planned day × {state.profile.trainingDays} days). Cardio days count distinct completed cardio days.</Text>
      </Card>

...
type ExerciseProgress = { bestE1rm: number; bestWeight: number; lastWeight: number; sessions: number; values: number[] };
...
export const ProgressScreen = () => {
  ...
  const exerciseProgress = useMemo(() => {
    ...
    completed.forEach((session) => session.exercises.forEach((exercise) => {
      ...
      if (!best) return;
      const current = map.get(exercise.exerciseId) ?? { bestE1rm: 0, bestWeight: 0, lastWeight: 0, sessions: 0, values: [] };
      const bestWeight = sets.reduce((value, set) => Math.max(value, set.weight), 0);
      map.set(exercise.exerciseId, {
        ...
        values: [...current.values, bestWeight]
      });
    }));
  ...
  if (selectedExerciseId) {
    ...
    const bestActualWeight = history.reduce((value, item) => Math.max(value, item.bestSet.weight), 0);
    ...
    return (
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Card style={styles.heroCard}>
          ...
          <Text style={styles.bigMetric}>{bestActualWeight || '—'} {state.profile.unit}</Text>

...
const defaultTrainingDayIndexes = (days: number) => days <= 2 ? [1, 4] : days === 3 ? [1, 3, 5] : days === 4 ? [1, 2, 4, 5] : [1, 2, 3, 5, 6];
...
export const TodayScreen = ({ openTrain, openHistory, openSettings }: { openTrain: () => void; openHistory: () => void; openSettings: () => void }) => {
  ...
  const [workoutEnvironment, setWorkoutEnvironment] = useState<'commercial' | 'home' | 'none'>('commercial');
  ...
  const completedToday = state.sessions.some((session) => !session.isCustom && session.completedAt && new Date(session.completedAt).toDateString() === new Date().toDateString());
  const allowedEquipment = workoutEnvironment === 'commercial' ? undefined : workoutEnvironment === 'home' ? new Set(['dumbbell', 'barbell', 'bodyweight', 'resistance-band', 'kettlebell']) : new Set(['bodyweight']);
  const customLibraryExercises = allowedEquipment ? exerciseLibrary.filter((exercise) => allowedEquipment.has(exercise.equipment)) : exerciseLibrary;
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      {completedToday ? (
        <Card style={styles.completedCard}>
          <Image source={require('../../assets/v7/workout_complete.png')} resizeMode="contain" style={styles.completedImage} />
          <Text style={styles.completedTitle}>Workout complete</Text>
          <Text style={styles.completedCopy}>Today’s scheduled session is finished. Your next workout will appear on its scheduled day.</Text>
        </Card>
      ) : (
      ...
      <View style={styles.openSection}>
        ...
        <View style={styles.goalGridTwo}>
          <View style={styles.goalCellTwo}><ProgressRing value={`${summary.workouts}/${workoutGoal}`} label="Total workouts" progress={workoutProgress} /></View>
          <View style={styles.goalCellTwo}><ProgressRing value={`${summary.cardioDays}/${cardioDaysGoal}`} label="Cardio days" progress={cardioDaysProgress} /></View>
        </View>
      ...
      <View style={styles.openSection}>
        ...
        <View style={styles.quickGrid}>
          ...
          <QuickMetric kind="calories" label="ACTIVE MINUTES" value={`${summary.activeMinutes}`} detail={`Goal: ${activeMinutesGoal} min`} />
          ...
          <View style={styles.previewSheet}>
            ...
            <ScrollView style={styles.previewList} contentContainerStyle={styles.previewListContent} showsVerticalScrollIndicator={false}>
              {template?.exercises.map((item, index) => {
                ...
                const loadIncrement = exercise?.defaultWeight === 0 ? 1 : 5;
                const displayWeight = roundToIncrement(blockAdjustedWeight(recommendation.weight, loadIncrement, blockWeek), loadIncrement);
      ...
      <Modal visible={showCustomBuilder} transparent animationType="slide" onRequestClose={() => setShowCustomBuilder(false)}>
        <View style={styles.modalBackdrop}>
          ...
          <View style={styles.customSheet}>
            ...
            <Text style={styles.customSectionTitle}>Workout environment</Text>
            <View style={styles.environmentRow}>
              {([['commercial', 'Commercial gym'], ['home', 'Home gym'], ['none', 'No gym']] as const).map(([id, label]) => (
                <Pressable key={id} onPress={() => { setWorkoutEnvironment(id); setShowCustomLibrary(true); }} style={[styles.environmentButton, workoutEnvironment === id && styles.environmentButtonActive]}>
                  <Text style={[styles.environmentText, workoutEnvironment === id && styles.environmentTextActive]}>{label}</Text>
                </Pressable>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  completedCard: { alignItems: 'center', padding: 18, borderColor: colors.accent, backgroundColor: '#07131F' },
  completedImage: { width: '100%', height: 220 },
  completedTitle: { color: colors.text, fontSize: 24, fontWeight: '900', marginTop: 4 },
  completedCopy: { color: colors.muted, fontSize: 11, lineHeight: 17, textAlign: 'center', marginTop: 5 },
  goalGridTwo: { flexDirection: 'row', gap: 14, justifyContent: 'center' },
  goalCellTwo: { flex: 1, alignItems: 'center' },
  environmentRow: { flexDirection: 'row', gap: 7, marginBottom: 10 },
  environmentButton: { flex: 1, minHeight: 40, borderRadius: 12, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.panelRaised, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 5 },
  environmentButtonActive: { borderColor: colors.accent, backgroundColor: colors.accentDeep },
  environmentText: { color: colors.muted, fontSize: 9, fontWeight: '900', textAlign: 'center' },
  environmentTextActive: { color: colors.accent },
